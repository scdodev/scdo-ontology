#creating_layperson_ontology_files_existing_lang_tags.py

#Program that adds layperson labels and descriptions to terms where necessary.
...
##### Adds layperson annotations where necessary and generates 2 files (1 with all ontology terms and 1 with only terms with layperson annotations)
def includes_layperson_annotations(owl_file_path, iri_path, layperson_tag, outputfile1_name, outputfile2_name):
     ...
     for term_ID in term_label_description_dict: #loops through the terms IDs in the "term_label_description_dict" dictionary (contains terms with layperson labels and descriptions)
          ...
          term_descriptions = getattr(term_object, "description") # obtains the list of descriptions ascribed to the term
          ...
          if term_ID in term_new_synonyms_dict: # if the term is in the dictionary for terms with new synonyms...
...
def main():
     ...
     ## Input Files ##
     layperson_annotations_file = "SCDO - Layperson_6May2020.txt" # path of .txt file containing layperson annotations
     owl_file_path = "scdo_29April2020.owl" # path of the owl file to which layperson annotations must be added (preferable to use rdf/xml file, not owl/xml file)
     
     ## Other variables ##
     layperson_annotations_file_cell1 = "ID" # content of the first cell in the input file containing layperson annotations
     iri_path = "http://scdontology.h3abionet.org/ontology/" # the specific generic IRI path that is used for terms in the ontology being translated
     layperson_tag = "en-x-lp" #specifies the tag to be added to all layperson annotations
     outputfile1_name = "entire_ontology_with_layperson_annotations_testing_with_en_added_v2.rdf" # name of rdf file created with all ontology terms
     outputfile2_name = "layperson_only_ontology_testing_with_en_added_v2.rdf" # name of rdf file created with only terms with layperson annotations
          
     #####Functions#####    
     reads_layperson_annotations(layperson_annotations_file, layperson_annotations_file_cell1)
     
     includes_layperson_annotations(owl_file_path, iri_path, layperson_tag, outputfile1_name, outputfile2_name)
